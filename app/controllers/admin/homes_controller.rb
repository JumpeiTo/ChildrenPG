class Admin::HomesController < ApplicationController
  ...
  def top
    ...
    @monthly_post_chart_data = Post.group_by_month_post_count

    # 日別の会員登録数を取得するクエリ（Customerモデルのスコープを使用）
    @daily_customer_chart_data = Customer.group_by_day_customer_count

    # 月別の会員登録数を取得するクエリ（Customerモデルのスコープを使用）
    @monthly_customer_chart_data = Customer.group_by_month_customer_count
  end
end
